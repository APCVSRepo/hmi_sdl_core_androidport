...
//    m_labelFrame.setGeometry((this->width() - iW) / 2, 0.85 * this->height() - iH, iW, iH);
    ...
    m_labelText1->setStyleSheet("border:0px;font: 42px \"Liberation Serif\";color:white;qproperty-alignment:AlignCenter;");
    m_labelText2->setStyleSheet("color:white;font: 42px \"Liberation Serif\";border-radius:8px;border-bottom:1px solid gray;background-color: #121122;"); //qproperty-alignment:AlignLeft;");
...
{
    AppBase::SetEdlidedText(m_labelText1,text,width()*0.9);
}

void CAudioPassThru::setAudioPassThruDisplayText2(QString text)
{
    AppBase::SetEdlidedText(m_labelText2,text,width()*0.9);
    //m_labelText2->setText(text);
}
...
void CAudioPassThru::showEvent(QShowEvent * e)
{
    if (AppControl)
    {        
        AppBase::SetEdlidedText(m_labelAppName,AppControl->getAppName().c_str(),width()*0.9);
